...
class Timer(commands.Cog):
    ...
    async def create_timer(self, interaction: discord.Interaction):
        ...
        message = await interaction.followup.send(f"Loading timer!")
        await message.edit(content=self.get_current_time_string())
        await self.bot.get_channel(self.bot.instances_channel).send(f"t {interaction.channel_id} {message.id}")
        self.bot.instances["timers"].append([interaction.channel_id, message.id])
    
    @tasks.loop(seconds=5.0)
    async def update_timers(self):
        for channel_id, message_id in self.bot.instances["timers"]:
